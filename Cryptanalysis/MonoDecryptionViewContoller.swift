                ...
                if Int(alphaKey)! % 2 != 0 && Int(alphaKey)! != 13 && Int(alphaKey)! < 26{
                    if Int(betaKey)! < 26{
                        if optionFlagAffine == 0{
                            resultTextView.text = monoDecryption.applyAffineEncryptionUsingKey(resultTextView.text.uppercaseString, alphaKey: Int(alphaKey)!, betaKey :Int(betaKey)!)
                        }else if optionFlagAffine == 1{
                            resultTextView.text = monoDecryption.applyAffineDecryptionUsingKey(resultTextView.text.uppercaseString, alphaKey: Int(alphaKey)!, betaKey: Int(betaKey)!)
                        }
                    }else{
